...
You also need to configure the environment for correct caching to work, for this you need to wrap your `next.config.js` in `withI18n`

```js
const { default: withI18n } = require('@nimpl/i18n/withI18n');

/** @type {import('next').NextConfig} */
const nextConfig = {};

module.exports = async (phase) => {
    const i18nWrapper = await withI18n(phase);
    return i18nWrapper(nextConfig);
}
```
